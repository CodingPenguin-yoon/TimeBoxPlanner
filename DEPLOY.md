...
컨테이너는 설정 검증 → `scripts/migrate.mjs` → 서버 실행 순서로 시작합니다. 설정이나 마이그레이션이 실패하면 서버를 실행하지 않습니다. 실행기는 대상 스키마를 명시하여 기존 `_prisma_migrations` 기록과 SQL 체크섬을 확인하고 미적용 SQL만 실행합니다. DB 잠금과 트랜잭션으로 동시 실행과 부분 적용을 방지합니다. 이미 적용된 SQL은 수정하지 않으며, 새 변경은 새 migration 디렉토리로 추가합니다. SQL 파일은 트랜잭션 안에서 실행할 수 있어야 하고 자체 BEGIN/COMMIT을 포함하지 않아야 합니다. `/api/health`는 사용자·계정·세션·플래너·할 일 테이블의 조회 권한까지 확인하고 성공 시 200, 실패 시 503을 반환합니다.

새 PostgreSQL DB에서 시작합니다. `prisma/migrations/`에는 테이블을 만드는 초기 스키마와 할 일 완료 상태 추가 마이그레이션이 포함되어 있습니다.

배포 후 확인:

1. `/api/health`가 200을 반환하는지 확인.
2. 로그아웃 상태에서 `/`는 로그인 화면으로 이동, `/api/planner`는 401인지 확인.
3. Google 로그인 후 홈에서 오늘의 플래너를 열고 할 일 작성 → 새로고침 → 같은 ID와 내용이 유지되는지 확인.
